...
def _rebuild_proxy_cache(source, python_runtime, cache_dir, metadata):
    ...
    try:
        ...
        raise UserError(
            f"Could not build the Windows helper proxy cache at {cache_dir}: "
            f"{error}.",
            "Close any running Cricut helper or Design Space process so the "
            "cached helper is not locked, then try again. You can also set "
            "SLICEBUG_DISABLE_WINDOWS_HELPER_PROXY=1 to skip the proxy and use "
            "the patch fallback.",
        ) from error
